...
def scatter_plot(name_a:str, data_a:np.array, name_b:str, data_b:np.array, name_hue:str=None, data_hue:np.array=None, 
    ...
    if data_hue is not None:
        ...
        if len(unique_values) == 1:
            sns.scatterplot(
                x=data_a.flatten(), y=data_b.flatten(), hue=data_hue.flatten(), palette=['green'], ax=ax)
        elif len(unique_values) == 2:
            sns.scatterplot(
                x=data_a.flatten(), y=data_b.flatten(), hue=data_hue.flatten(), palette=['green', 'red'], ax=ax)
        elif len(unique_values) == 3:
            sns.scatterplot(
                x=data_a.flatten(), y=data_b.flatten(), hue=data_hue.flatten(), palette=['green', 'yellow', 'red'], ax=ax)
        else:
            sns.scatterplot(x=data_a.flatten(), y=data_b.flatten(), hue=data_hue.flatten(), palette=cmapGR, ax=ax)
    else:
        show_legend = False
        sns.scatterplot(x=data_a.flatten(), y=data_b.flatten(), ax=ax)
...
def confusion_matrix_plot(name_a:str, data_a:np.array, name_b:str, data_b:np.array, title:str, axis:plt.axis=None, path:str=None):
    '''
    ...
    data_a, data_b = data_a.astype(str), data_b.astype(str)
    values = np.unique(data_a)
    cmat = confusion_matrix(data_a.flatten(), data_b.flatten(), labels=values)
    sns.heatmap(pd.DataFrame(cmat, values, values), annot=True, linewidths=.5, fmt='d', cmap='Blues_r', ax=ax)
...
def swarm_plot(features:dict, data_hue:np.array=None, title:str=None, x_label:str='Sensitivity', y_label:str='Features', 
    ...
    sensitivity = np.concatenate(list(features.values()), axis=0) if len(features) > 1 else np.array(list(features.values())).flatten()
    feature_names = np.array([[n]*len(d) for n, d in features.items()]).flatten().astype(str)
    ...
    if data_hue is not None:
        ...
        if len(sensitivity) > len(features):
            if len(sensitivity) / len(features) <= MAX_SWARMPLOT_SAMPLES:
                sns.swarmplot(x=sensitivity.astype(float), y=feature_names, hue=hue, size=4, edgecolor='gray', palette=palette, ax=ax)
            else:
                sns.stripplot(x=sensitivity.astype(float), y=feature_names, hue=hue, size=4, jitter=0.25, edgecolor='gray', palette=palette, ax=ax)
        else:
            sns.barplot(x=sensitivity.astype(float), y=feature_names, color='b', ax=ax)
    else:
        if len(sensitivity) > len(features):
            if len(sensitivity) / len(features) <= MAX_SWARMPLOT_SAMPLES:
                sns.swarmplot(x=sensitivity.astype(float), y=feature_names, size=4, edgecolor='gray', palette='flare', ax=ax)
            else:
                sns.stripplot(x=sensitivity.astype(float), y=feature_names, size=4, edgecolor='gray', jitter=0.25, palette='flare', ax=ax)
        else:
            sns.barplot(x=sensitivity.astype(float), y=feature_names, color='b', ax=ax)
    ...
    if data_hue is not None and data_hue.dtype in ['int', 'uint', 'float'] and len(sensitivity) > len(features):
        handles, labels = ax.get_legend_handles_labels()
    ...
    if plot_statistics and len(sensitivity) > len(features):
        sns.boxplot(x=sensitivity.astype(float), y=feature_names, whis=np.inf, palette=sns.color_palette("pastel"), boxprops=dict(facecolor=(0,0,0,0)), linewidth=1)
...
def imshow_many(images:List[np.array], title:str, row_width:int=3, axis:plt.axis=None, path:str=None):
    '''
